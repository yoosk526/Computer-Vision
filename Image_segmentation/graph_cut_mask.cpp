...
void graph_cut_mask(Mat input, Mat maskImg) {
	...
	for (y = 0; y < height; y++) {
		for (x = 0; x < width; x++) {
			if (maskImg.at<Vec3b>(y, x)[0] >= 255 - ErrRange && maskImg.at<Vec3b>(y, x)[1] <= ErrRange && maskImg.at<Vec3b>(y, x)[2] <= ErrRange) 
			{
				mask.at<uchar>(y, x) = GC_BGD;
				//printf("bg(%d, %d)\t\t", x, y);
			}
			else if (maskImg.at<Vec3b>(y, x)[0] <= ErrRange && maskImg.at<Vec3b>(y, x)[1] <= ErrRange && maskImg.at<Vec3b>(y, x)[2] >= 255 - ErrRange) 
			{
